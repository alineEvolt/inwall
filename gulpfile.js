...
gulp.task('browserSync', function() {
  browserSync({
    proxy: {
      target: 'http://localhost:8888/INWALL/landing/wordpress/'
    }
  })
})
...
gulp.task('imagesUI', function() {
  return gulp.src('app/css/vendor/images/**/*.+(png|jpg|jpeg|gif|svg)')
    // Caching images that ran through imagemin
    .pipe(cache(imagemin()))
    .pipe(gulp.dest('dist/css/vendor/images'))
});
...
gulp.task('default', function(callback) {
  runSequence(['sass', 'vendorjs', 'vendorcss', 'jsuse', 'browserSync', 'watch'],
    callback
  )
})
...
gulp.task('build', function(callback) {
  runSequence(
    'clean:dist',
    'sass',
    ['vendorjs', 'vendorcss', 'useref', 'images', 'imagesUI'],
    callback
  )
})